...
import {
  ...
  MessageStrip, BusyIndicator, Toolbar, ToolbarSpacer, Select, Option, StepInput, DatePicker,
...
export function StockTransferPage() {
  ...
  const { data: items } = useSapQuery<{ value: any[] }>(
    'Items?$select=ItemCode,ItemName&$filter=Frozen eq tNO',
    { enabled: !!selectedTenantId }
  );
  ...
  async function handlePost() {
    ...
    const result = await mutate('StockTransfers', body, 'POST');
    if (result.success) {
      setMessage({ text: 'Umlagerung gebucht', type: 'Positive' });
      setLines([]);
    } else {
  ...
  return (
    <div>
      <Toolbar>
        <Title level="H3">Umlagerungsanfrage</Title>
        <ToolbarSpacer />
        <Button design="Emphasized" icon="save" onClick={handlePost} disabled={lines.length === 0}>Buchen</Button>
      </Toolbar>

      {message && <MessageStrip design={message.type} onClose={() => setMessage(null)} style={{ marginBottom: '0.5rem' }}>{message.text}</MessageStrip>}

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', padding: '1rem', background: 'var(--sapGroup_TitleBackground)', borderRadius: '0.25rem', marginBottom: '1rem' }}>
        <div>
          <Label required>Quelllager</Label>
          <Select onChange={(e: any) => setFromWarehouse(e.detail.selectedOption.value)} style={{ width: '100%' }}>
            <Option value="">-- Quelllager --</Option>
            {warehouseList.map((w: any) => <Option key={w.WarehouseCode} value={w.WarehouseCode} selected={w.WarehouseCode === fromWarehouse}>{w.WarehouseCode} – {w.WarehouseName}</Option>)}
          </Select>
        </div>
        <div>
          <Label required>Ziellager</Label>
          <Select onChange={(e: any) => setToWarehouse(e.detail.selectedOption.value)} style={{ width: '100%' }}>
            <Option value="">-- Ziellager --</Option>
            {warehouseList.filter((w: any) => w.WarehouseCode !== fromWarehouse).map((w: any) => <Option key={w.WarehouseCode} value={w.WarehouseCode} selected={w.WarehouseCode === toWarehouse}>{w.WarehouseCode} – {w.WarehouseName}</Option>)}
          </Select>
        </div>
        <div>
          <Label>Buchungsdatum</Label>
          <Input type="Date" value={docDate} onInput={(e: any) => setDocDate(e.target.value)} style={{ width: '100%' }} />
        </div>
      </div>
      ...
      >
        {lines.map(line => (
          <TableRow key={line.itemCode}>
            ...
            <TableCell><Button design="Negative" icon="delete" onClick={() => setLines(p => p.filter(l => l.itemCode !== line.itemCode))} /></TableCell>
          </TableRow>
        ))}
      </Table>
    </div>
