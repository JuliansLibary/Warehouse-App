...
export function AppShell() {
  ...
  const { selectedTenantName, selectedWarehouseName, pendingCount } = useSelector(
    (s: RootState) => ({ ...s.tenant, pendingCount: s.offline.pendingCount })
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      <ShellBar
        logo={<img src="/nag-logo.png" alt="nAG" style={{ height: 32 }} />}
        primaryTitle="Warehouse App"
        secondaryTitle={selectedTenantName ?? ''}
        profile={
          <ui5-avatar
        ...
        startButton={<ShellBarItem icon="menu2" text="Menu" />}
      >
        {selectedWarehouseName && (
          <ShellBarItem icon="factory" text={selectedWarehouseName} />
        )}
        {pendingCount > 0 && (
          <ShellBarItem icon="synchronize" text={`Sync (${pendingCount})`} />
        )}
      </ShellBar>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <SideNavigation
          collapsed={false}
          style={{ width: 240, height: '100%', borderRight: '1px solid var(--sapList_BorderColor)' }}
